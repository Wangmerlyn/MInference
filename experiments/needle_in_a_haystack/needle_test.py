...
class Config:
    ...
    def __post_init__(self):
        ...
        output_file = f"needle_res_{os.path.basename(self.model_name)}_{self.run_name if self.run_name is not None else ''}_{self.jobs if self.jobs is not None else ''}_{timestamp}_{self.context_lengths_min}_{self.context_lengths_max}_{self.pattern_path.split('/')[-1].replace('.json', '') if self.pattern_path is not None else ''}.json"
...
def main(
    ...
    kwargs = {
        ...
        "tensor_parallel_size": int(os.getenv("niah_tp_size", "4")),
    }
